...
function renderSnapshotLine(row) {
  ...
  if (row?.type) domMeta += `[type="${row.type}"]`
...
async function buildSnapshot(state, request) {
  ...
  const snapshotResult = await page.evaluate(({ markerAttr, maxRefs, candidateLimit, mode }) => {
    ...
    for (const el of rawCandidates) {
      ...
      const isMediaOrFrame = tag === 'canvas' || tag === 'iframe' || tag === 'video' || tag === 'audio'
      const label = ariaLabel || placeholder || title || alt || labelText || text
      if (!isMediaOrFrame && !label) continue
      ...
      const isLongText = ['div', 'li', 'ul', 'p', 'span', 'section'].includes(tag) && label.length > 40
      if (isLongText && mode !== 'full') continue
